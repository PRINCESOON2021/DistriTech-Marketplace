...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    verify_csrf();
    if (login_rate_limited()) { http_response_code(429); $error='Trop de tentatives. Réessayez dans 15 minutes.'; }
    elseif (AdminAuth::login((string)($_POST['email']??''),(string)($_POST['password']??''))) { clear_login_failures(); header('Location: index.php'); exit; }
    else { register_login_failure(); usleep(350000); $error='Identifiants incorrects.'; }
}
